...
#
# Build:
#   docker build -f Dockerfile --build-arg GIT_COMMIT_TRIGGER=$(date +%s) --build-arg SSH_KEY="$(cat ~/.ssh/id_ed25519)" -t mpfrxx .
#
# Run:
#   docker run -it --rm mpfrxx
#
# Development:
#   docker run -it --rm -v $(pwd):/workspace mpfrxx
#
# Performance profiling with perf:
#   sudo sysctl -w kernel.perf_event_paranoid=-1
#   docker run -it --rm --privileged mpfrxx
...
RUN apt install -y gdb valgrind google-perftools linux-tools-generic linux-headers-generic
